...
use crate::waveform::drawer::DrawerWithCache;
    ...
    fn ui(self, ui: &mut egui::Ui) -> egui::Response {
        ...
        if ui.is_rect_visible(resp.rect) {
            ...
            drawer
                ...
                .draw(ui, resp.rect, self.offset_points, self.color);
        }
        ui.memory_mut(|mem| {
            let drawer_publisher = mem.caches.cache::<DrawerPublisher<'_>>();
...
mod drawer {
    ...
    use crate::waveform::{WaveData, WaveformPaintMode};

    const MAX_TEXTURE_WIDTH: usize = 1024;

    pub struct DrawerWithCache {
    ...
    impl DrawerWithCache {
        ...
        pub fn draw(
            ...
            for texture_index in visible_texture_range.clone() {
                let texture_id =
                    if let Some(texture_id) = self.saved_texture_ids.get(&texture_index) {
                        *texture_id
                    } else {
        ...
        fn draw_texture_multiple_pixels_per_sample(
            &self,
            texture_index: usize,
        ) -> egui::ColorImage {
        ...
        fn draw_texture_multiple_samples_per_pixel(
            &self,
            texture_index: usize,
        ) -> egui::ColorImage {
            let mut image = egui::ColorImage::filled(
            ...
            let drawer = Self::value_drawer(match self.mode {
                WaveformPaintMode::LinePoint | WaveformPaintMode::LineStep => {
                    WaveformPaintMode::Bar
                }
                other => other,
            });
            ...
            for i in 0..MAX_TEXTURE_WIDTH {
                ...
                for sample_index in (start_sample_index..end_sample_index)
                    .step_by(self.data.channels.get() as usize)
                {
                    let Some(sample) = self.data.samples_interleaved.get(sample_index) else {
                        continue;
                    };
